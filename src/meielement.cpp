...
mei::MeiElement::~MeiElement() { this->attributes.clear(); }

mei::MeiElement::MeiElement(const MeiElement &ele)
    : name(ele.name), value(ele.value), tail(ele.tail), parent(ele.parent),
      document(NULL) {
  ...
  // use default MeiAttribute copy constructor with shallow pointer copy for
  // MeiElement
  vector<MeiAttribute *>::const_iterator attr_it;
  for (attr_it = ele.attributes.begin(); attr_it != ele.attributes.end();
       ++attr_it) {
...
bool mei::MeiElement::operator==(const mei::MeiElement &other) const {
  return (this->id != "" && this->name == other.name && this->id == other.id);
}
...
const string mei::MeiElement::getId() { return this->id; }

void mei::MeiElement::setId(string id) { this->id = id; }

bool mei::MeiElement::hasId() { return this->id != ""; }

const string mei::MeiElement::getName() { return this->name; }

const string mei::MeiElement::getValue() { return this->value; }

void mei::MeiElement::setValue(string value) { this->value = value; }

const string mei::MeiElement::getTail() { return this->tail; }

void mei::MeiElement::setTail(string tail) { this->tail = tail; }
...
void mei::MeiElement::setAttributes(const vector<MeiAttribute *> attrs) {
  ...
  for (vector<MeiAttribute *>::const_iterator i = attrs.begin();
       i != attrs.end(); ++i) {
    this->addAttribute(*i);
  }
}
...
MeiAttribute *mei::MeiElement::getAttribute(string name) {
  for (vector<MeiAttribute *>::iterator iter = attributes.begin();
...
bool mei::MeiElement::hasParent() { return parent != NULL; }

void mei::MeiElement::setParent(MeiElement *parent) { this->parent = parent; }

mei::MeiElement *mei::MeiElement::getParent() { return this->parent; }

void mei::MeiElement::setDocument(MeiDocument *document) throw(
    DocumentRootNotSetException) {
  if (!document->getRootElement()) {
    throw DocumentRootNotSetException(
        "The document root is not set. Please set it using "
        "MeiDocument::setRootElement()");
  }
  ...
  for (vector<mei::MeiElement *>::iterator iter = descendants.begin();
       iter != descendants.end(); ++iter) {
    (*iter)->setDocument(document);
  }
}

MeiDocument *mei::MeiElement::getDocument() { return this->document; }
...
void mei::MeiElement::removeDocument() {
  ...
  for (vector<mei::MeiElement *>::iterator iter = children.begin();
...
void mei::MeiElement::addChildBefore(MeiElement *before, MeiElement *child) {
  child->setParent(this);
  vector<MeiElement *>::iterator pos =
      find(children.begin(), children.end(), before);
  if (pos == children.end()) {
    return;
  }
...
const vector<mei::MeiElement *>
mei::MeiElement::getChildrenByName(string name) {
  vector<mei::MeiElement *> res;
  for (vector<MeiElement *>::iterator iter = this->children.begin();
...
vector<mei::MeiElement *> mei::MeiElement::getDescendantsByName(string name) {
  ...
  for (vector<MeiElement *>::iterator iter = desc.begin(); iter != desc.end();
       ++iter) {
    if ((*iter)->getName() == name) {
      res.push_back(*iter);
    }
...
void mei::MeiElement::printElement() { printElement(0); }
...
const vector<mei::MeiElement *> mei::MeiElement::flatten() {
  ...
  for (vector<mei::MeiElement *>::const_iterator iter = children.begin();
       iter != children.end(); ++iter) {
...
mei::MeiCommentNode::MeiCommentNode() : MeiElement("_comment") {}
